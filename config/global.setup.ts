...
test('Loginand store Session', async ({ loginPage }) => {
    const USERNAME = process.env.VALID_USER ?? (() => { throw new Error('VALID_USER is not defined'); })();
    const PASSWORD = process.env.VALID_PASSWORD ?? (() => { throw new Error('VALID_PASSWORD is not defined'); })();
